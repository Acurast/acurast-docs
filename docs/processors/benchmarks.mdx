...
> ⚠️ The compute score formula and weights are currently under development and may change in the future. As a starting point, all components are weighted equally, but the scoring method will be improved over time to better reflect real-world needs.

> ⚠️ The $score_{storage}$ metric is currently inactive and does not affect the compute score at this time.
...
Where:
$$
w(x_{t}) = x_{t} \times W_{t}
$$
And:
- $W_{t}$: Weight assigned to each component ($0 \leqslant W_{t} \leqslant 1$ and $\sum_{t} W_{t} = 1$)
...
$$
W_{t} = \frac{1}{n} \text{ if active, or } W_{t} = 0 \text{ otherwise}
$$
Where:
- $n$: The number of active metrics