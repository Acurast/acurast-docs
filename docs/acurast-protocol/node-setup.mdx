...
Immediately after the call succeeds, change `--rpc-methods unsafe` back to `--rpc-methods safe` and recreate the container with `docker compose up -d`. Confirm that the RPC endpoint is not publicly accessible before continuing.
