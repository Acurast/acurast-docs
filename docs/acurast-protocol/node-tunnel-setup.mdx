...
## Deploying a node with the integrated tunnel server

This document walks through running an Acurast Canary Node with the built-in QUIC
tunnel server enabled. It is self-contained: every CLI flag, port, cert mode,
and proxy rule needed to bring a tunnel enabled node is described inline.

### Overview

The tunnel server relays end-user TCP traffic to Acurast processors. Clients
...
A working `docker-compose.yml` for an tunnel enabled deployment:
