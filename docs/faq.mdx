...
Depending on the type of token you get (EVM or Acurast native), you might need to bridge it first, before you can send it to the wallet you use for Acurast Mainnet. See token section: [Acurast Token](/token).
...
As long as your phone is connected to the internet, and the Processor application app is running, you get a reward. You can increase the rewards by staking your ACU tokens, see [Staked Compute](/staked-compute/overview).
