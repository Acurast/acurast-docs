...
Compute is measured through four benchmark tests conducted on every device running the Acurast processor. These tests run automatically during each device heartbeat (every 30 minutes) and the results are reported on-chain. At least one valid benchmark report per epoch (900 blocks / roughly 90 minutes) is required to maintain a device's measured compute. Read more about the technical aspects of the benchmarks [here](/processors/benchmarks).
...
The share of rewards each committer gets depends on the following factors, which can be chosen or influenced by the committer when creating the stake. The reward split is calculated per Benchmark Metric Pool:

`weight = benchmark_score * stake_amount * cooldown`
