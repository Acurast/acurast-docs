...
slug: /token-holders/claiming-acu
---

Some entities and individuals have received ACU tokens that are vested for a certain period of time, as outlined in the [token allocations](/investors/tokenomics#token-allocations). Vested tokens originate from early funding rounds, team allocations, and airdrops. As time passes, the owners can claim the released tokens.
