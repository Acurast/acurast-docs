 ...
 * @param {P2PError} error the error callback.
 */
_STD_.p2p.connect(peer, success, error);
 ...
 * @return {Promise<number>} a promise that resolves with the actual number of bytes read.
 */
type P2PStreamRead = (n) => Promise<number>;
