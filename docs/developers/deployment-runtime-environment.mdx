 ...
 * 2. **Reverse DNS + TXT** — for each resolved IP, performs a PTR lookup and
 *    verifies that `_acu.<ptr_host>` carries a TXT record
 *    `v=base64(sha256(deployment_source || host))`.
 *
