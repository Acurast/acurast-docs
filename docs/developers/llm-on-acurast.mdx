...
We need to set the `LOCALTUNNEL_SUBDOMAIN` variable to specifiy where our server will be available. If we set it to `llm`, then the URL will be `https://llm.proxy.acu.run`.
...
If you followed this tutorial, then your app will be available at `https://<your-subdomain>.proxy.acu.run`. ("\<your-subdomain\>" is the value you set for `LOCALTUNNEL_SUBDOMAIN` in the code).
