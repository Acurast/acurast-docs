...
Exposes the deployment's reverse tunnel to job scripts. The tunnel forwards inbound TLS connections (terminated at `https://<clientId>.<domainSuffix>:8443`) to a local address inside the deployment. See [Tunnel quick start](/developers/getting-started/quickstart-tunnel) for the DNS records that must be in place on `<domainSuffix>` before calling `start`.
