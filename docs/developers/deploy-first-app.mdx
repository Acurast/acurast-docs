...
We need to set the `LOCALTUNNEL_SUBDOMAIN` variable to specifiy where our server will be available. If we set it to `example`, then the URL will be `https://example.acu.run`.
