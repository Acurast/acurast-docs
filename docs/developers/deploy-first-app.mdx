...
- **[Runtime Environment](/developers/build/runtime-environment)** — what's available to your script on the processor.
- **[Deployment Config](/developers/build/deployment-config)** — full `acurast.json` reference.
- **[Example Apps](/developers/getting-started/examples)** — more projects to clone and learn from.
