...
The tunnel maps `https://<clientId>.<your-suffix>` to your deployment, so you need
a DNS suffix where a wildcard record and an `_acu` TXT record have been published.
This is a one-time setup — follow the
[Tunnel Quick Start](/developers/getting-started/quickstart-tunnel)
(step 2) to configure it. In this example the suffix is `tunnel.acurast.dev`.
...
| `DOMAIN_SUFFIX_MAINNET` / `DOMAIN_SUFFIX_CANARY` | ✅ (active one) | Your tunnel DNS suffix. Set only the one matching `NETWORK`. |
...
- `includeEnvironmentVariables` — the allowlist of `.env` vars forwarded to the
  processor (`CALLBACK_URL`, `SSH_PASSWORD`, `DOMAIN_SUFFIX_MAINNET`, `NETWORK`).
