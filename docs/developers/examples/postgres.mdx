...
The tunnel needs a DNS suffix with a wildcard record and an `_acu` TXT record
published — a one-time setup covered by the
...
- `includeEnvironmentVariables`: `CALLBACK_URL`, `DOMAIN_SUFFIX_MAINNET`,
  `NETWORK`, `SSH_PASSWORD`, `POSTGRES_USER/PASSWORD/DB`, `WEB_PORT`.
