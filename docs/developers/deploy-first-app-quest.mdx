...
1. Open up the [Acurast Console](https://console.acurast.com/) and click on "Create Deployments". 
...
Once the start time is reached, your app will be available at `https://<processor-address>.processor-proxy.sook.ch`. (Note: processor addresses are all lowercase.). You can find the address of the processor that runs your app in the deployment details in the Acurast Console. Open the "Deployments" view, then click on the deployment and look at the "Assigned Processors" section. Copy the "Main Acurast Account", this is the processor address that runs your app.
