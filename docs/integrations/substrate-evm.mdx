...
The example shows how to route the fulfillment's payload to a smart contract by calling the `fulfill` mehod on it and passing the payload bytes are argument.
