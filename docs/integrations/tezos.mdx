...
class EntropyProvider(sp.Contract):
    def __init__(self):
        self.init_type(
            sp.TRecord(
                ...
                entropy=sp.TBytes,
                last_update=sp.TTimestamp,
                processors=sp.TSet(sp.TAddress),
                subscriptions=sp.TBigMap(sp.TAddress, sp.TTimestamp)
            ).right_comb()
        )
    ...
    def subscribe(self, address):
        ...
        mutez_to_seconds = sp.fst(sp.ediv(sp.amount, sp.mutez(1)).open_some(sp.unit))
        expiration = sp.add(sp.now, sp.to_int(mutez_to_seconds))
        self.data.subscriptions[address] = expiration

    @sp.entry_point(new_syntax=True)
    ...
    def consume_entropy(self, max_age_in_seconds):
        ...
        subscription = sp.compute(self.data.subscriptions.get(sp.sender, message = "NOT_SUBSCRIBED"))
        sp.verify(subscription >= sp.now, "SUBSCRIPTION_EXPIRED")
        # Consumers can specify the maximum entropy age they are willing to accept
        sp.verify((sp.now - self.data.last_update) <= max_age_in_seconds, "ENTROPY_TOO_OLD")
...
def test():
    ...
    contract.update_initial_storage(initial_storage)
...
sp.add_compilation_target(
    "EntropyProviderCompilation",
    contract=EntropyProvider(),
    storage=sp.record(
        governance_address = sp.address("tz1XvqmBUa7SkUFRHSygUZsxMwh7i8GpV7iB"),
        entropy = sp.bytes("0x"),
        last_update = sp.timestamp(0),
        processors = sp.set([]),
        subscriptions = sp.big_map()
...
The script below computes some random bytes securely and calls the function `entropy` of the contract deployed above. The Consumer can also specify the gas limit and maximum costs per gas unit.
