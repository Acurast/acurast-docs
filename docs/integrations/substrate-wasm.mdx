...
The javascript job that can fulfill to such a contract looks like the following:
...
httpGET(
    ...
    (response, _certificate) => {
        ...
        const payload = _STD_.chains.substrate.codec.encodeCompactUnsignedNumber(1) + _STD_.chains.substrate.codec.encodeBytes(key) + _STD_.chains.substrate.codec.encodeUnsignedNumber(price, 128);
