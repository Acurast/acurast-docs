...
Acurast can be used to get access to computational resources for various use cases and bring them to EVM chains. For that, the `Developer` creates a Deployment that performs a given task on the [Acurast Console](https://console.acurast.com/).
...
In this example, the contract expects the Acurast Processor to be known in advance, this can also be adapted at a later stage, depending on how you want to assign your Deployment to one or more Processors.
...
contract EntropyProvider {
    ...
    /**
     * Receive entropy (random bytes) from an Acurast deployment.
     */
...
The script below computes some random bytes securely and calls the function `receive_entropy` of the contract deployed above. The Developer can also specify the gas limit and maximum costs per gas unit.
