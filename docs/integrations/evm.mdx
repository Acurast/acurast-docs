...
Acurast can be used to get access to computational ressources for various use cases and bring them to EVM chains. For that, the `Consumer` creates a Job that performs a given task on the Acurast Marketplace.
...
Currently, the Job registration process requires some technical knowledge which may be difficult to understand by non developer users. The process will be simplified in a future release of https://console.acurast.com.

At this stage, you can register a Job on [https://polkadot.js.org](https://polkadot.js.org/apps/?rpc=wss://wss.collator-1.acurast.papers.tech/#/extrinsics), follow these steps:
